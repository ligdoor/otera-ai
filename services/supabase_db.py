...
def get_temple_by_name(temple_name):
    """寺院名で検索"""
    try:
        response = supabase.table('temples').select('*').eq('name', temple_name).execute()
...
def update_temple(temple_name, temple_data):
    """寺院データを更新"""
    try:
        response = supabase.table('temples').update(temple_data).eq('name', temple_name).execute()
        return True
    except Exception as e:
        print(f"寺院更新エラー: {e}")
        return False

def add_temple(temple_data):
    """寺院データを追加"""
    try:
        response = supabase.table('temples').insert(temple_data).execute()
        return True
    except Exception as e:
        print(f"寺院追加エラー: {e}")
        return False
...
def get_all_temples():
    """全寺院データを取得"""
    try:
        response = supabase.table('temples').select('*').execute()
