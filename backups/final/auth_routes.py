...
def admin():
    ...
    if request.method == "POST":
        ...
        if not can_login:
            add_log("ログイン失敁E, f"user_id: {user_id} - {error_msg}", request.remote_addr)
        ...
        if user_name:
            ...
            add_log("ログイン失敁E, f"user_id: {user_id} - 認証エラー", request.remote_addr)
            return """<script>alert('IDまた�Eパスワードが違いまぁE); window.location.href='/admin';</script>"""
...
def logout():
    ...
    add_log("ログアウチE, f"{user_name} がログアウトしました")
    session.clear()
    return redirect(url_for('auth.admin'))
...
def _change_password_supabase(user_id, current_pass, new_pass):
    ...
    if is_valid:
        ...
        add_log("パスワード変更失敁E, "現在のパスワードが間違ってぁE��ぁE)
        return jsonify({"message": "現在のパスワードが間違ってぁE��ぁE}), 400

def _change_password_sheets(user_id, current_pass, new_pass):
    """Google Sheets版�Eパスワード変更"""
...
def render_login_page():
    """ログインペ�EジのHTMLを返す�E�連打防止機�E付き�E�E""
    return f"""
    <html>
    <head>
    ...
    <body>
        <div class="login-container">
            ...
            <form method="post" id="login-form">
                ...
                <input type="password" name="password" placeholder="パスワーチE required autocomplete="current-password" id="password-input">
            ...
            <div class="security-note">
                <strong>⚠�E�EセキュリチE��</strong><br>
                3回失敗で警告通知<br>
                5回失敗で5刁E��ロチE��<br>
                30刁E��操作で自動ログアウチE            </div>
            <a href="/" class="back-link">ↁEアプリへ戻めE/a>
        </div>
        
        <script>
            // ☁Eログインボタンの連打防止�E�修正版！E☁E            const loginForm = document.getElementById('login-form');
            ...
            loginForm.addEventListener('submit', function(e) {{
                ...
                // 送信フラグを立てめE                isSubmitting = true;
                
                const button = document.getElementById('login-button');
                const spinner = document.getElementById('loading-spinner');
                
                // ボタンのみ無効化（�E力欁E�E無効化しなぁE��E��E                button.disabled = true;
                button.textContent = '処琁E��...';
